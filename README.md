...
Coverage: Download (File structure) → First launch (UI & program behaviors) → First encode (Showing flow) → Second encode (Showing tools) | 涵盖范围：下载（文件结构）→ 首次启动（UI 与软件行为）→ 首次压制（展示流程）→ 再次压制（展示工具）
