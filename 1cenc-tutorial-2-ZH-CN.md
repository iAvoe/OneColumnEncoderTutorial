...
> **注意：**以上是纯经验判断（往难听了说就是瞎猜），并非分类规则标准
...
**最终播放列表：**
<img src="./img2-all/3-BDPlaylistSelector-1.png" alt="蓝光播放列表选择器 2" width=700 />

**完成队列导入：**
<img src="./img2-all/3-BDPlaylistSelector-2.png" alt="蓝光播放列表选择器 3" width=500 />

> 同时，这个空格 + 双引号的恐怖路径被正确解析了
...
由于历史原因，在 1080p 分辨率下，原始蓝光标准仅允许：
- 1080p @ 23.976 / 24 fps（逐行，面向电影）
- 1080i @ 29.97 / 25 fps（隔行，即 59.94i / 50i，面向电视节目）
- 1080p @ 59.94 / 50 fps（不在标准内，直到 Ultra HD 4K 蓝光才加入）

因此，大部分蓝光播放机会拒绝播放 30fps 逐行扫描的源。这产生了两种兼容策略：
- 通过 SPS/PPS 标记隔行解码方案，实现伪 29.97fps
  - 实际为逐行扫描
  - 可以无损重编码，但据说存在一些“笨播放器”，会强行施加额外的去隔行处理，导致画质损失
- 编码为隔行，并且可能存在 3:2 或其它格式的 Pulldown，实现 1080i 29.97 格式
  - 需要逐帧观察，识别 Pulldown 规律，选用正确的恢复滤镜组

1cenc 对 Pulldown 恢复处理的能力有限，因此暂且需要古法处理，此处略。
